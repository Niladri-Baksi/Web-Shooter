...
splats = []          # each entry: {"x": int, "y": int, "timer": int, "max_timer": int}
MAX_SPLATS = 2        # how many webs can be on screen at once
SPLAT_LIFETIME = 90   # ~3 sec at 30fps, tune to taste
...
def overlay_image(background, overlay, x, y,alpha_scale=1.0):
    h, w = overlay.shape[:2]
    ...
    alpha = (overlay_crop[:, :, 3] / 255.0) * alpha_scale
...
while True:
    ...
    if len(lmList) != 0:
        ...
        THUMB_EXTEND_THRESH = 2.0   # must exceed this to count as "extended"
        THUMB_RETRACT_THRESH = 1.6  # must drop below this to count as "retracted" again

        thumb_pinky_ratio = thumb_to_pinky / hand_size

        if thumb_pinky_ratio > THUMB_EXTEND_THRESH:
            thumb_extended = True
        elif thumb_pinky_ratio < THUMB_RETRACT_THRESH:
            thumb_extended = False
        else:
            thumb_extended = previous_thumb   # in the dead zone, hold the last state
        ...
        if web_active:
            ...
            if web_distance > 500:
                web_active = False
                splats.append({"x": web_x, "y": web_y, "timer": SPLAT_LIFETIME, "max_timer": SPLAT_LIFETIME})
                if len(splats) > MAX_SPLATS:
                    splats.pop(0)   # drop the oldest one
    ...
    for splat in splats[:]:          # iterate a copy since we may remove while looping
        fade = splat["timer"] / splat["max_timer"]   # 1.0 = fresh, 0.0 = about to vanish

        web_h, web_w = web_image.shape[:2]
        ox = splat["x"] - web_w // 2
        oy = splat["y"] - web_h // 2
        img = overlay_image(img, web_image, ox, oy, alpha_scale=fade)
